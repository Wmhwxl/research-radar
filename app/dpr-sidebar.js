...
(function () {
  ...
  function parseSidebar(text) {
    ...
    while (i < lines.length) {
      ...
      // 顶层入口（首页/教程）
      if (/^\*\s+<a\b/.test(trimmed) && i < 4) {
        var top = parseTopLink(trimmed);
        if (top) {
          if (!model.home && (top.href === '#/' || /\/$/.test(top.href))) {
            model.home = top;
          } else if (/research-radar/.test(top.href)) {
            model.researchRadar = top;
  ...
  function renderSidebarHeader(homeHref, tutorialHref, homeLabel, tutorialLabel, radarHref, radarLabel) {
    return (
      ...
      (radarHref ? renderQuickLink('dpr-sidebar-quick-radar', radarHref, '◎', radarLabel || 'Research Radar') : '') +
  ...
  function renderShell(root) {
    ...
    var radarHref = state.model.researchRadar && state.model.researchRadar.href;
    var radarLabel = state.model.researchRadar && state.model.researchRadar.label;
    ...
    root.innerHTML =
      ...
      renderSidebarHeader(homeHref, tutorialHref, homeLabel, tutorialLabel, radarHref, radarLabel) +
